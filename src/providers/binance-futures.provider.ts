...
/**
 * Binance USDT-margined perpetual futures. Same public, key-free,
 * official real-time WebSocket model as spot (binance.provider.ts), just
 * pointed at the futures API/stream hosts. Canonical symbol id is
 * prefixed `BINANCEFUTURES:` (distinct from `BINANCE:` spot) since the
 * underlying ticker string is identical for spot and perp (e.g. BTCUSDT).
 */
...
export class BinanceFuturesProvider implements MarketProvider {
  ...
  readonly name = 'Binance Futures WebSocket (USDT-M)';
  readonly isRealtime = true;
  readonly experimental = false;
  readonly categories: MarketCategory[] = ['crypto_futures'];
  ...
  async connect(): Promise<void> {
    ...
    this.connectPromise = new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => reject(new Error('Binance Futures WS connect timeout')), 8000);
      ...
      this.ws.on('open', () => {
        ...
        if (this.subscribed.size > 0) {
          this.ws?.send(JSON.stringify({ method: 'SUBSCRIBE', params: [...this.subscribed], id: Date.now() }));
        }
        this.startWatchdog();
        resolve();
      });
  ...
  async getQuote(symbol: string): Promise<Quote | null> {
    ...
    const res = await fetch(`${REST_BASE}/fapi/v1/ticker/24hr?symbol=${streamSymbol}`);
  ...
  async getCandles(symbol: string, interval: CandleInterval, limit = 200): Promise<Candle[]> {
    ...
    const res = await fetch(
      `${REST_BASE}/fapi/v1/klines?symbol=${streamSymbol}&interval=${INTERVAL_MAP[interval]}&limit=${limit}`,
    );
    if (!res.ok) return [];
  ...
  async healthCheck(): Promise<ProviderHealth> {
    ...
    try {
      const res = await fetch(`${REST_BASE}/fapi/v1/ping`);
  ...
  private handleMessage(raw: string): void {
    ...
    try {
      ...
      const quote: Quote = {
        symbol: `BINANCEFUTURES:${(data.s as string).toUpperCase()}`,
        price: Number(data.p),
        change: null,
        changePercent: null,
